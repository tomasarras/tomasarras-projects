...
const ChartItem = ({ src, title, children, size, activeChart }) => (<>
  <div className={`${(activeChart == null || activeChart == title) ? styles.active : ""}`}>
    <span className={`text-2xl font-bold ${title == "devops" ? styles.devopsLabel : title == 'backend' ? styles.backendLabel : styles.frontendLabel}`}>{children}</span>
    <Image
      src={src}
      alt="semicircle"
      width={size}
...
export default function Chart({ size, activeChart }) {
  ...
  <div className={styles.container}>
    <ChartItem src="/chart_pie_1.svg" title="devops" size={size} activeChart={activeChart}>DevOps</ChartItem>
