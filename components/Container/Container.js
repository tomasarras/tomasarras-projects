...
export default function Container({ children, page, center = false, noCenter = false, noPadding = false }) {
  const { currentPage } = useContext(Context);
  const isActive = page == undefined || page == currentPage;

  return (
  <section className={`${noPadding ? "" : "container"} sm:py-4 h-100 ${s.section} ${!isActive && s.invisible}`}>
