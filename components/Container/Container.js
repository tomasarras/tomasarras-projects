...
export default function Container({ children, page, fullContainerClassName, center = false, noCenter = false, noPadding = false, className = "" }) {
  const { currentPage } = useContext(Context);
  const isActive = page == undefined || page == currentPage;

  return (
  <section className={`${fullContainerClassName} w-full h-full`}>
    <div className={`${noPadding ? "" : "container"} sm:py-4 h-100 ${s.section} ${!isActive && s.invisible} ${className}`}>
